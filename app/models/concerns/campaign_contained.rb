module CampaignContained
  ...
  included do
    ...
    accepts_nested_attributes_for :campaign_contents, allow_destroy: true, :reject_if => lambda { |contents| contents['campaign_id'].blank? }
  ...
  def in_campaign?(campaign_id)
    ...
    ( klass.where("#{table}.id = #{self.id}").in_campaign(campaign_id).count > 0 )
  end
